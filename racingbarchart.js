...
d3.json("games_sales_yearly.json", (dr) => {
  ...
  for (let i = 0; i < numGames; i++) {
    ...
    let value = dr.NA_Sales[i];
    let yearData = data.find(d => d.year == yr);
    if (yearData) {
      let platformData = yearData.dataSet.find(d => d.name == platform);
      if (platformData) {
        platformData.value += value;
      }
      else {
        if (platform != null && value != null) {
          yearData.dataSet.push({name: platform, value: value})
        }
      }
    }
    else {
      data.push({year: yr, dataSet: []})
    }
  }    
